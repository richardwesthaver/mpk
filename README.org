...
* Overview
=mpk= is my /Media Programming Kit/. It provides a development kit for
digital media. Batteries are *not* included -- =mpk= is a flexible
ecosystem designed to organize your creative workflow which may
include your own hardware, software, and digital assets.

This project is quite young and will only deal with audio for quite
some time since that's the medium I'm most interested in. Emacs is the
only GUI available for =mpk=, but a CLI is also provided.

The =mpk= APIs and programs are primarily written in Rust but there
are a number of other langs involved including C, Nim, and Python.

** Features
- Resource Management
  - SQLite DB
  - index tracks, samples, sessions, patches, plugins, MIDI
  - query metadata and audio features
- JACK
  - Session/Patch support
  - Utility clients
  - Network Audio
- HTTP Clients
  - freesound.org
  - musicbrainz.org
- Emacs Frontend
  - JACK patching and session manager
  - EMMS playlists
  - Supercollider/Lilypond IDE
  - MIDI/OSC keymaps
  - Org-mode notes/tasks
- Hardware Support
  - build and upload sample chains for Elektron devices
  - send/receive SysEx
- Configuration
  - global config =mpk.toml=
  - session config =session.toml=
- Distribution
  - Compress(zst)/Archive(tar) support
  - backup/dump/restore DB
** Dependencies
- Rust
- Python
  - essentia
...
**** TODO retrieve sample_id correctly
should never insert id of 0 when duplicate found, update or noop instead
**** TODO account for missing values in essentia
...
*** TODO mpk_config updates [0/3]
**** TODO add support for external paths
