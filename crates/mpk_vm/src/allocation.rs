...
use crate::util::*;
use crate::{object::*, thread::immix_get_tls_state};
...
pub trait Allocator {
  ...
  /// collections.
  fn get_all_blocks(&mut self) -> alloc::vec::Vec<*mut Block>;
...
impl NormalAllocator {
  ...
  pub fn set_recyclable_blocks(&mut self, blocks: Vec<*mut Block>) {
    self.recyclable_blocks = blocks;
  }
}

impl Allocator for NormalAllocator {
  fn get_all_blocks(&mut self) -> Vec<*mut Block> {
    ...
    if size >= LINE_SIZE {
      ...
      match self.recyclable_blocks.pop() {
        ...
        Some(block) => {
          ...
          match unsafe { (*block).scan_block((size_of::<Block>() - 1) as u16) } {
            None => {
              self.handle_full_block(block);
              self.handle_no_hole(size)
            }
            Some((low, high)) => {
              debug_assert!(low as usize >= size_of::<Block>());
...
impl EvacAllocator {
  ...
  pub fn extend_evac_headroom(&mut self, blocks: impl IntoIterator<Item = *mut Block>) {
...
impl ImmixSpace {
  ...
  pub fn filter(&self, addr: Address) -> Option<Address> {
    ...
    if unsafe { (*self.block_allocator).is_in_space(addr) }
      && self.bitmap.test(addr.to_usize())
    {
      return Some(addr);
    }
  ...
  pub fn new(heap_size: usize) -> *mut Self {
    unsafe {
      ...
      let bitmap = SpaceBitmap::<16>::create(
        "immix space bitmap",
        (*block).mmap.start(),
        heap_size,
      );
  ...
  pub fn return_blocks(&mut self, blocks: impl IntoIterator<Item = *mut Block>) {
    unsafe {
  ...
  /// collections.
  pub fn get_all_blocks(&mut self) -> Vec<*mut Block> {
  ...
  pub unsafe fn maybe_evacuate(&mut self, addr: *mut RawGc) -> Option<Address> {
    let block_info = Block::get_block_ptr(Address::from_ptr(addr));
