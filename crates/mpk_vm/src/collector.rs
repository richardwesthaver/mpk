      ...
      unsafe {
        ...
        if !(&mut *object).mark(next_live_mark) {
          if immix_space.filter_fast(object_addr) {
            let block = Block::get_block_ptr(object_addr);
...
impl Collector {
  pub fn new() -> Self {
    Self {
      all_blocks: Vec::new(),
      mark_histogram: VecMap::with_capacity(LINE_COUNT),
    }
  }
  /// Store the given blocks into the buffer for use during the collection.
  pub fn extend_all_blocks(&mut self, blocks: Vec<*mut Block>) {
  ...
  /// blocks.
  fn sweep_all_blocks(&mut self) -> (Vec<*mut Block>, Vec<*mut Block>) {
  ...
  fn establish_hole_threshhold(&self, evac_headroom: usize) -> usize {
    let mut available_histogram: VecMap<usize> = VecMap::with_capacity(LINE_COUNT);
    ...
    let mut available_lines = evac_headroom * (LINE_COUNT - 1);

    for threshold in 0..LINE_COUNT {
