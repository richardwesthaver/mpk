...
Simply run =nim install -d:py= in the project root to install the mpk
binary in =~/.cargo= and the python modules in the default location
(usually =/usr/local/lib/python3.9/site-packages=).
