...
pub const OT_FILE_HEADER: [u8; 23] = [0x46,0x4F,0x52,0x4D,0x00,0x00,0x00,0x00,0x44,0x50,0x53,0x31,0x53,0x4D,0x50,0x41, 0x00,0x00,0x00,0x00,0x00,0x02,0x00];
...
impl SampleChain {
  ...
  // TODO
  pub fn process_file<P: AsRef<Path>>(&mut self, path: P, even_spacing: bool) -> Result<(), std::io::Error> {
    let path = path.as_ref();
    eprintln!("Processing file: {:?}", path);
    let file = std::fs::File::open(path).unwrap();
    // if input.sample_rate() != self.sample_rate 
    //   || input.channels() != self.channels {
    let source = UniformSourceIterator::<_, i16>::new(Decoder::new(BufReader::new(file)).unwrap(), self.channels, self.sample_rate);
//      }
    ...
    let new_slice = ChainSlice {start_point: self.start_offset, length: slice_len, loop_point: slice_len};
