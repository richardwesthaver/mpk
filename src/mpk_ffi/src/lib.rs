...
#[no_mangle]
pub unsafe extern "C" fn mpk_string_free(ptr: *mut c_char) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
/// single int.
#[no_mangle]
pub unsafe extern "C" fn mpk_db_config_flags(cfg: *const Config) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_audio_data_new(
  path: *const c_char,
  filesize: usize,
  duration: f64,
  channels: u8,
...
#[no_mangle]
pub unsafe extern "C" fn mdb_audio_data_free(ptr: *mut AudioData) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_track_tags_new(
  artist: *const c_char,
  title: *const c_char,
  album: *const c_char,
  genre: *const c_char,
  ...
  let tags = TrackTags {
    artist: if artist.is_null() {
      ...
      Some(CStr::from_ptr(artist).to_str().unwrap().to_string())
    },
    ...
    album: if album.is_null() {
      ...
      Some(CStr::from_ptr(album).to_str().unwrap().to_string())
    },
    genre: if genre.is_null() {
      None
    } else {
      Some(CStr::from_ptr(genre).to_str().unwrap().to_string())
    },
    date: if date.is_null() {
      None
    } else {
      Some(CStr::from_ptr(date).to_str().unwrap().to_string())
    },
    tracknumber: if tracknumber.is_null() {
      None
    } else {
      Some(CStr::from_ptr(tracknumber).to_str().unwrap().to_string())
    },
    format: if format.is_null() {
      None
    } else {
      Some(CStr::from_ptr(format).to_str().unwrap().to_string())
    },
    language: if language.is_null() {
      None
    } else {
      Some(CStr::from_ptr(language).to_str().unwrap().to_string())
    },
    country: if country.is_null() {
      None
    } else {
      Some(CStr::from_ptr(country).to_str().unwrap().to_string())
    },
    label: if label.is_null() {
      None
    } else {
      Some(CStr::from_ptr(label).to_str().unwrap().to_string())
    },
    producer: if producer.is_null() {
      None
    } else {
      Some(CStr::from_ptr(producer).to_str().unwrap().to_string())
    },
    engineer: if engineer.is_null() {
      None
    } else {
      Some(CStr::from_ptr(engineer).to_str().unwrap().to_string())
    },
    mixer: if mixer.is_null() {
      None
    } else {
      Some(CStr::from_ptr(mixer).to_str().unwrap().to_string())
    },
  };
...
#[no_mangle]
pub unsafe extern "C" fn mdb_track_tags_free(ptr: *mut TrackTags) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_musicbrainz_tags_new(
  ...
  let tags = MusicbrainzTags {
    ...
    albumstatus: if albumstatus.is_null() {
      ...
      Some(CStr::from_ptr(albumstatus).to_str().unwrap().to_string())
    },
    albumtype: if albumtype.is_null() {
      None
    } else {
      Some(CStr::from_ptr(albumtype).to_str().unwrap().to_string())
    },
    ...
    asin: if asin.is_null() {
      ...
      Some(CStr::from_ptr(asin).to_str().unwrap().to_string())
    },
...
#[no_mangle]
pub unsafe extern "C" fn mdb_musicbrainz_tags_free(ptr: *mut MusicbrainzTags) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_lowlevel_features_free(ptr: *mut LowlevelFeatures) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_rhythm_features_free(ptr: *mut RhythmFeatures) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_sfx_features_free(ptr: *mut SfxFeatures) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_tonal_features_new(
  ...
  let features = TonalFeatures {
    ...
    chords_key: CStr::from_ptr(chords_key).to_str().unwrap().to_string(),
    chords_scale: CStr::from_ptr(chords_scale).to_str().unwrap().to_string(),
...
#[no_mangle]
pub unsafe extern "C" fn mdb_tonal_features_free(ptr: *mut TonalFeatures) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_spectrograms_free(ptr: *mut Spectrograms) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_new(path: *const c_char) -> *mut Mdb {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_free(ptr: *mut Mdb) {
  if ptr.is_null() {
    return;
  }
  Box::from_raw(ptr);
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_track_tags(
  db: *const Mdb,
  id: i64,
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_track_tags_musicbrainz(
  db: *const Mdb,
  id: i64,
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_track_features_lowlevel(
  db: *const Mdb,
  id: i64,
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_track_features_rhythm(
  db: *const Mdb,
  id: i64,
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_track_features_sfx(
  db: *const Mdb,
  id: i64,
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_track_features_tonal(
  db: *const Mdb,
  id: i64,
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_track_images(
  db: *const Mdb,
  id: i64,
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_sample_features_lowlevel(
  db: *const Mdb,
  id: i64,
  features: *const LowlevelFeatures,
) {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_sample_features_rhythm(
  db: *const Mdb,
  id: i64,
  features: *const RhythmFeatures,
) {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_sample_features_sfx(
  db: *const Mdb,
  id: i64,
  features: *const SfxFeatures,
) {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_sample_features_tonal(
  db: *const Mdb,
  id: i64,
  features: *const TonalFeatures,
) {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_insert_sample_images(
  db: *const Mdb,
  id: i64,
  images: *const Spectrograms,
) {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_update_path(
  db: *const Mdb,
  path: *const c_char,
  ty: *const c_char,
) {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_query_check_file(
  db: *const Mdb,
  path: *const c_char,
  ty: *const c_char,
) -> *mut c_char {
...
#[no_mangle]
pub unsafe extern "C" fn mdb_exec_batch(db: *const Mdb, sql: *const c_char) {
  let sql = CStr::from_ptr(sql).to_str().unwrap();
  let mdb = &*db;
