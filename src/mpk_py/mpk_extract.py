...
def run():
    ...
    parser.add_argument("-j", "--jobs", default=mp.cpu_count(), help="number of parallel jobs")
...
def prep_common(v):
  ...
  audiodata = audio_data(metadata["path"][0],
...
def insert(data):
    for k,v in data.items():
        if args.db:
            if args.type == "track":
              (audiodata, lowlevel, rhythm, sfx, tonal, specs, tags, mb_tags) = prep_track(v)
              insert_track(audiodata, lowlevel, rhythm, sfx, tonal, specs, tags, mb_tags)
            elif args.type == "sample":
              (audiodata, lowlevel, rhythm, sfx, tonal, specs) = prep_track(v)
